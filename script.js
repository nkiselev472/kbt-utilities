...
class UI {
        ...
        buttons.forEach(button => {
            button.addEventListener('click', (e) => {
                if (button.classList.contains('disabled') || 
                    Date.now() - state.lastClickTime < 300) {
                    e.preventDefault();
                    e.stopPropagation();
                    return;
                }
    ...
    static renderTransferHistory(data = state.transferData) {
        const now = Date.now();
        if (now - state.lastRenderTime < 100) {
            return;
        }
                ...
                try {
                    QRCode.toCanvas(canvas, qr.text, {
                        ...
                        if (error) {
                            ...
                            UI.createFallbackQR(canvas, qr.text);
                        }
                    });
                } catch (error) {
    ...
    static createFallbackQR(canvas, text) {
        ...
        ctx.font = '10px Arial';
        ctx.fillText(this.truncateText(text, 15), size/2, size/2 + 15);
    }
    ...
    static loadQRCodeLibrary() {
        if (typeof QRCode !== 'undefined') {
            ...
            Logger.add('Библиотека QRCode уже загружена', 'info');
            return Promise.resolve();
        }
        
        // Проверяем, загружена ли библиотека через тег script
        const scripts = document.querySelectorAll('script');
        const qrScriptLoaded = Array.from(scripts).some(script => 
            script.src.includes('qrcode') && script.src.includes('.js')
        );
        
        if (qrScriptLoaded) {
            // Даем время библиотеке загрузиться
            return new Promise((resolve) => {
                setTimeout(() => {
                    if (typeof QRCode !== 'undefined') {
                        state.isQRCodeLibraryLoaded = true;
                        Logger.add('Библиотека QRCode загружена', 'success');
                    }
                    resolve();
                }, 500);
            });
...
class Scanner {
    static async checkCameraPermission() {
        try {
            ...
            Logger.add('Ошибка доступа к камере', 'error', { error: error.message });
            return false;
        }
    }
    ...
    static async start(mode) {
        if (state.isScanning) {
            Logger.add('Сканер уже запущен', 'warn');
            return;
        }
        ...
        try {
            ...
            await state.activeScanner.start(
            ...
            if (error.name === 'NotAllowedError') {
                ...
                errorMsg = 'Попробуйте переключить камеру';
                // Пробуем запустить с любой камерой
                try {
                    await state.activeScanner.start(
                        { facingMode: "user" },
                    ...
                    if (startBtn) startBtn.disabled = true;
                    if (stopBtn) stopBtn.disabled = false;
                    UI.showNotification('Сканер запущен (передняя камера)', 'success');
                    return;
                } catch (e) {
                    errorMsg = 'Не удалось запустить ни одну камеру';
                }
            }
    ...
    static playBeep() {
        try {
            // Используем более простой способ для звука
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Ждем загрузки библиотеки QRCode
    setTimeout(() => {
        UI.renderTransferHistory();
        UI.renderQRCodesGallery();
    }, 500);
