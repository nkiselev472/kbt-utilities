...
class Scanner {
    ...
    static getScannerConfig() {
        ...
        return {
            fps: isIOS ? 5 : 10, // Уменьшаем FPS для iOS
            qrbox: { 
                width: 250, 
                height: 250 
            },
            aspectRatio: 1.0,
            disableFlip: isIOS, // Важно для iOS
            experimentalFeatures: {
                useBarCodeDetectorIfSupported: !isIOS // Отключаем для iOS
            }
        };
    }
    ...
    static async start(mode) {
        ...
        // Проверяем разрешение на камеру перед запуском
        if (!await this.checkCameraPermission()) {
            UI.showNotification('Разрешите доступ к камере в настройках браузера', 'error');
            return;
        }
